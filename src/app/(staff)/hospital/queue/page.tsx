...
export default function HospitalQueuePage() {
  ...
  useEffect(() => {
    ...
    if (q.length === 0) {
      q = [
        { id: "Q-104", facility: "District Hospital", patientName: "Vikram Mehta", time: "10:00 AM", status: "waiting", date: new Date().toLocaleDateString() },
        { id: "Q-105", facility: "District Hospital", patientName: "Sneha Desai", time: "10:30 AM", status: "waiting", date: new Date().toLocaleDateString() },
      ]
    ...
    const fetchQueue = () => {
      ...
      // Fetch appointment requests for District Hospital
      const reqs = getAppointmentRequests().filter(r => r.facility === "District Hospital")
  ...
  const hospitalQueue = queue.filter(q => q.facility === "District Hospital" && q.status === "waiting")
  const nextPatient = hospitalQueue.length > 0 ? hospitalQueue[0] : null

  const handleConfirmAppointment = (req: AppointmentRequest) => {
    const assignedTime = assignTimeMap[req.id] || "10:30 AM"
    const staffNote = noteMap[req.id] || "Schedule confirmed by District Hospital. Please arrive 15 mins early."
    ...
    if (!q.some((item: any) => item.patientName === req.patientName && item.facility === "District Hospital")) {
       q.push({
    ...
    setAppointmentReqs(getAppointmentRequests().filter(r => r.facility === "District Hospital"))
  }
  ...
  const handleComplete = (actionType: "treated" | "admitted") => {
    ...
    history.push({
      ...
      actionTaken: actionType === "treated" ? "Treated in OPD: " + notes : "Admitted to IPD/Ward",
      status: actionType === "treated" ? "Treated" : "Admitted"
    })
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 pb-12">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
           <div className={`bg-white rounded-3xl shadow-2xl w-full overflow-hidden animate-in zoom-in-95 duration-200 transition-all ${consentStatus === 'approved' ? 'max-w-4xl' : 'max-w-lg'}`}>
              ...
              <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-blue-50">
                 <div>
                    <h2 className="text-xl font-bold text-slate-900">OPD Patient Processing</h2>
                      ...
                      <>
                        ...
                        <div className="space-y-2">
                          <input 
                            ...
                            className="w-full text-center text-lg p-4 border-2 border-slate-200 rounded-xl focus:outline-none focus:border-blue-500 font-mono font-bold uppercase tracking-widest bg-slate-50"
                          />
                        ...
                        <Button disabled={!consentCode} onClick={() => handleRequestConsent()} className="w-full bg-blue-600 hover:bg-blue-700 text-white py-6 text-lg rounded-xl font-bold">
                       ...
                       <button 
                         ...
                         className="text-slate-500 hover:text-blue-700 font-semibold text-sm mt-4 underline decoration-slate-300 underline-offset-4"
                       >
                ...
                <>
                  <div className="p-8 space-y-6 max-h-[70vh] overflow-y-auto">
                     <div>
                        ...
                        <textarea 
                           className="w-full h-24 p-3 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm"
                     ...
                     <div>
                        ...
                        <div className="mt-4">
                           <input 
                              ...
                              className="w-full p-3 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm bg-slate-50"
                           />
                        </div>
                     </div>
                     ...
                     <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-slate-100">
                        <div>
                           <label className="block text-sm font-semibold text-slate-700 mb-2 flex items-center gap-2"><FlaskConical size={16} className="text-blue-600"/> Request Diagnostics</label>
                           <input 
                              list="hospital-queue-diag-options"
                              className="w-full p-3 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-slate-50"
                              placeholder="Type a test name or select from list..."
                              value={selectedDiag}
                              onChange={e => setSelectedDiag(e.target.value)}
                           />
                           <datalist id="hospital-queue-diag-options">
                  ...
                  <div className="p-6 border-t border-slate-100 flex flex-col sm:flex-row justify-end gap-3 bg-slate-50">
                     <Button onClick={() => handleComplete('treated')} className="bg-blue-600 hover:bg-blue-700 text-white px-8"><CheckCircle2 className="w-4 h-4 mr-2" /> Treat & Complete</Button>
                     <Button onClick={() => handleComplete('admitted')} className="bg-slate-900 hover:bg-slate-800 text-white px-8">Admit Patient to ICU <ArrowRight className="w-4 h-4 ml-2" /></Button>
                  </div>
                </>
              )}
           </div>
      ...
      <div>
        <h1 className="text-3xl font-bold tracking-tight text-slate-900">OPD Queue & Scheduling Management</h1>
      ...
      {appointmentReqs.length > 0 && (
         <Card className="shadow-lg border-2 border-indigo-500/30 bg-indigo-50/20">
            <CardHeader className="bg-white border-b border-indigo-100 py-5">
               <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                     ...
                     <div>
                        <CardTitle className="text-xl text-indigo-950">Patient Day Appointment Requests</CardTitle>
            ...
            <CardContent className="p-6 space-y-4 bg-white">
               {appointmentReqs.map(req => {
                  ...
                  return (
                     <div key={req.id} className="p-5 border-2 border-indigo-100 rounded-2xl bg-indigo-50/10 space-y-4">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-1">
                           <div>
                              ...
                              <input 
                                 type="text" 
                                 placeholder="e.g. 10:30 AM or 02:30 PM"
                                 value={assignTimeMap[req.id] !== undefined ? assignTimeMap[req.id] : (req.confirmedTime || "10:30 AM")}
                                 onChange={e => setAssignTimeMap(prev => ({ ...prev, [req.id]: e.target.value }))}
                                 className="w-full p-2.5 border border-slate-200 rounded-xl text-sm font-bold bg-slate-50 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                              />
                           </div>
                           <div>
                              <label className="text-xs font-bold text-slate-700 uppercase tracking-wider block mb-1.5">
                              ...
                              <input 
                                 type="text" 
                                 placeholder="e.g. Report to Cardiology OPD Desk #2"
                                 value={noteMap[req.id] !== undefined ? noteMap[req.id] : (req.facilityNote || "Schedule confirmed by District Hospital.")}
                                 onChange={e => setNoteMap(prev => ({ ...prev, [req.id]: e.target.value }))}
                                 className="w-full p-2.5 border border-slate-200 rounded-xl text-sm font-medium bg-slate-50 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                              />
                           </div>
                        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
         ...
         <div className="md:col-span-2 space-y-6">
            <Card className="shadow-lg border-2 border-blue-500 overflow-hidden relative">
         ...
         <div className="space-y-6">
            <Card className="shadow-sm">
               <CardContent className="p-6">
                  ...
                  <div className="text-5xl font-extrabold text-slate-900">{hospitalQueue.length}</div>
