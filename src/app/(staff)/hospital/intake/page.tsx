...
export default function HospitalIntakePage() {
  ...
  const [lastAction, setLastAction] = useState<{ type: "treated" | "admitted" | "error"; name: string } | null>(null)
    ...
    const fetchState = () => {
       ...
       if (currentConsent && consentStatus === "requesting") {
          if (currentConsent.status === "approved") {
             ...
             setPatientName("Anjali Desai (Fetched via ABHA)")
  ...
  useEffect(() => {
     if (consentStatus === "approved") {
        if (isManualOverride) {
           ...
           // Check for active referrals to this hospital
           const refs = JSON.parse(localStorage.getItem("medrelay.staffReferrals") || "[]")
           const inboundRefs = refs.filter((r: any) => 
             r.stages.some((s: any) => (s.status === "current" || s.status === "pending") && (s.facility === "District Hospital" || s.facility.includes("Cardiology") || s.facility.includes("Orthopedics")))
           )
  ...
  const handleComplete = (type: "treated" | "admitted") => {
    if (selectedDiag) {
    ...
    history.push({
      ...
      actionTaken: type === "treated" ? "Treated in OPD" : "Admitted to IPD/Ward",
      status: type === "treated" ? "Treated" : "Admitted"
    })
          ...
          if (selectedReferralsToClose.includes(r.id)) {
             const newStages = r.stages.map((s: any) => {
                if (s.status === "current" || s.status === "pending") {
                   return { ...s, status: "done", note: type === "treated" ? "Treated in OPD" : "Admitted to IPD/Ward", date: new Date().toLocaleString() }
                }
                return s
             })
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div>
        <h1 className="text-3xl font-bold tracking-tight text-slate-900">Hospital OPD Intake</h1>
        <p className="text-slate-500 mt-1">Register outpatient walk-ins. Digital ABDM consent or manual registration required.</p>
      </div>
                       ...
                       <div className="grid grid-cols-2 gap-4">
                          ...
                          <div>
                             ...
                             <select 
                                ...
                                className="w-full mt-1.5 h-10 px-3 border border-slate-200 rounded-md text-sm font-medium focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white"
                             >
                      ...
                      <div className="max-w-md mx-auto space-y-5">
                        ...
                        <div>
                           <button 
                             ...
                             className="text-slate-500 hover:text-blue-700 font-semibold text-sm mt-3 underline decoration-slate-300 underline-offset-4"
                           >
        ...
        {consentStatus === "approved" && (
           <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
             ...
             <div className="lg:col-span-7 space-y-6">
               <Card className="shadow-lg border-2 border-blue-500/20">
                 <CardHeader className="bg-blue-50 border-b border-blue-100 flex flex-row items-center gap-4 py-6">
                   <div className="w-12 h-12 rounded-full bg-blue-600 text-white flex items-center justify-center shadow-inner shrink-0">
                      <UserPlus size={24} />
                   </div>
                   <div>
                      <CardTitle className="text-2xl text-blue-900">Patient File: {patientName}</CardTitle>
                 ...
                 <CardContent className="p-6 sm:p-8 space-y-6 bg-white">
                   ...
                   <div className="space-y-4">
                      <Label className="text-base font-bold text-slate-800 flex items-center gap-2">
                         <FlaskConical className="text-blue-600" size={18} /> Order OPD Diagnostics / Tests
                      </Label>
                      <select 
                        value={selectedDiag} 
                        onChange={e => setSelectedDiag(e.target.value)}
                        className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl text-slate-800 font-medium focus:outline-none focus:ring-2 focus:ring-blue-500"
                      >
                   ...
                   <div className="pt-6 border-t border-slate-100 flex flex-col sm:flex-row gap-4">
                      ...
                      <Button 
                        ...
                        className="flex-1 bg-blue-600 hover:bg-blue-700 text-white py-6 text-base font-bold rounded-xl shadow-lg shadow-blue-600/20"
                      >
                         <UserPlus size={20} className="mr-2" /> Admit Patient to Ward (IPD)
                      </Button>
                   </div>
                   ...
                   <CardHeader className="bg-slate-50 border-b border-slate-200 p-5">
                      ...
                      <div className="mt-3">
                         ...
                         <select
                            ...
                            className="w-full p-2.5 bg-white border-2 border-slate-200 rounded-xl text-xs font-bold text-slate-800 shadow-xs focus:outline-none focus:border-blue-500"
                         >
                            ...
                            <option value="referred">🚑 Referrals & Hospital Admissions</option>
                            ...
                            >
                               ...
                               {rec.diagnostics && (
                                  <div className="pt-2 border-t border-slate-100 flex items-center justify-between gap-2">
                                     <div className="flex items-center gap-1.5 text-blue-800 font-bold text-[11px] truncate">
                                     ...
                                     <button
                                        ...
                                        className="bg-blue-600 hover:bg-blue-700 text-white text-[11px] font-extrabold px-3 py-1.5 rounded-lg shadow-xs flex items-center gap-1.5 transition-colors shrink-0 cursor-pointer"
                                     >
               ...
               <div className="bg-blue-700 text-white p-5 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                     ...
                     <div>
                        ...
                        <p className="text-xs text-blue-200">Verified via ABDM Health Information Exchange</p>
                     </div>
                  </div>
                  <button 
                     onClick={() => setSelectedReportModal(null)} 
                     className="text-blue-200 hover:text-white p-1.5 rounded-lg hover:bg-blue-800 transition-colors"
                  >
                     <X size={20} />
                  </button>
               </div>
               ...
               <div className="p-6 space-y-5 text-sm">
                  ...
                  <div className="grid grid-cols-2 gap-4 bg-slate-50 p-4 rounded-xl border border-slate-200 text-xs">
                     <div>
                        ...
                        <span className="font-bold text-slate-900">{patientName || "Anjali Desai"} (PT-8891)</span>
                     </div>
                     <div>
                     ...
                     <div>
                        ...
                        <span className="font-mono font-bold text-blue-700">REP-{(selectedReportModal.id || "1042").slice(-6).toUpperCase()}</span>
                     </div>
                  </div>
                        ...
                        <table className="w-full text-left text-xs">
                           ...
                           <tbody className="divide-y divide-slate-100 text-slate-800 font-medium">
                              <tr>
                                 ...
                                 <td className="p-3 font-bold text-blue-900">{selectedReportModal.reportDetails?.value || "Normal Findings"}</td>
                  ...
                  {/* Verification Footer */}
                  <div className="flex items-center gap-2 text-xs text-blue-700 bg-blue-50 p-3 rounded-xl border border-blue-200 font-medium">
            ...
            <div className="flex items-center gap-3">
               ...
               <div>
                  ...
                  <p className="text-xs text-slate-400">
                     Status: {lastAction.type === "treated" ? "Treated in OPD" : "Admitted to Hospital IPD"}
                  </p>
               </div>
            </div>
