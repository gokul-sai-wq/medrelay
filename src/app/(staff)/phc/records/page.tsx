...
export default function PHCPatientRecords() {
  ...
  const loadData = () => {
    ...
    setRecords(history.filter((h: any) => 
      h.facility === "Villianur PHC, Pondicherry" || 
      h.facility === "Villianur Sub-Centre PHC, Pondicherry" ||
      h.facility.includes("PHC") ||
      h.facility.includes("Sub-Centre")
    ))
  ...
  const handleCloseReferralSubmit = (e: React.FormEvent) => {
    ...
    closeReferral(closingRefId, closureNote.trim() || "Consultation finalized and referral closed by PHC team.", "Villianur PHC, Pondicherry")
  ...
  const exportRecords = () => {
    ...
    dlAnchor.setAttribute("download", `medrelay-phc-patient-records-${new Date().toISOString().slice(0, 10)}.json`)
    dlAnchor.click()
  }
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-3xl font-extrabold tracking-tight text-slate-900">PHC Patient Records & Referral Registry</h1>
          <p className="text-slate-500 text-sm mt-1">
            Longitudinal electronic health records, outbound referrals, and tertiary hospital closure continuity for Villianur PHC.
          </p>
        </div>
        <Button onClick={exportRecords} className="bg-teal-600 hover:bg-teal-700 text-white shadow-sm flex items-center gap-2">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-xs">
          <p className="text-xs font-bold text-slate-500 uppercase tracking-wider">Total PHC Encounters</p>
        ...
        <div className="bg-white p-4 rounded-2xl border border-amber-200 bg-amber-50/20 shadow-xs">
          <p className="text-xs font-bold text-amber-700 uppercase tracking-wider">Active Referrals</p>
        ...
        <div className="bg-white p-4 rounded-2xl border border-emerald-200 bg-emerald-50/20 shadow-xs">
          <p className="text-xs font-bold text-emerald-700 uppercase tracking-wider">Closed Referrals</p>
          <p className="text-2xl font-black text-emerald-800 mt-1">
            {combinedRecords.filter(r => r.referralId && r.isRefClosed).length}
          </p>
        </div>
        <div className="bg-white p-4 rounded-2xl border border-blue-200 bg-blue-50/20 shadow-xs">
          <p className="text-xs font-bold text-blue-700 uppercase tracking-wider">Medical Histories</p>
        ...
        <CardHeader className="bg-slate-50 border-b pb-4 space-y-4 rounded-t-xl">
          <div className="flex flex-col md:flex-row items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-1.5 p-1 bg-slate-200/70 rounded-xl w-full md:w-auto">
              ...
              >
                All Records ({combinedRecords.length})
              </button>
              <button
                type="button"
            ...
            <div className="relative w-full md:w-72">
              ...
              <input 
                ...
                className="w-full pl-9 pr-4 py-2 bg-white border border-slate-200 rounded-xl text-xs font-medium focus:outline-none focus:ring-2 focus:ring-teal-500" 
              />
            </div>
        ...
        <CardContent className="p-0">
          {filteredRecords.length === 0 ? (
             <div className="p-16 text-center text-slate-500">
                <Activity size={44} className="mx-auto mb-3 opacity-25 text-teal-600" />
                <p className="font-bold text-base text-slate-700">No matching patient records found.</p>
              ...
              <table className="w-full text-left border-collapse min-w-[1100px]">
                <thead>
                  <tr className="border-b text-xs text-slate-500 uppercase tracking-wider font-extrabold bg-slate-100/60">
                    ...
                    <th className="p-4 py-3.5">Medical History & Symptoms</th>
                    <th className="p-4 py-3.5">Prescriptions & Diagnostics</th>
                    <th className="p-4 py-3.5">Referral Pathway</th>
                  ...
                  {filteredRecords.map((record, i) => {
                    ...
                    return (
                      <tr key={i} className="hover:bg-slate-50/80 transition-colors">
                        ...
                        {/* COLUMN 2: SEPARATE MEDICAL HISTORY & SYMPTOMS */}
                        <td className="p-4 align-top max-w-xs">
                          <div className="font-bold text-slate-900 leading-snug">
                            {record.chiefComplaint || "Routine Outpatient Clinical Consultation"}
                          </div>
                        ...
                        {/* COLUMN 3: PRESCRIPTIONS & DIAGNOSTICS */}
                        <td className="p-4 align-top max-w-xs">
                          <div className="text-xs text-slate-700 leading-relaxed font-medium">
                            {record.actionTaken || "Clinical consultation and care guidelines provided."}
                          </div>
                          {record.diagnostics && (
                            <div className="text-xs font-bold text-indigo-700 bg-indigo-50 border border-indigo-100 px-2.5 py-1 rounded-lg flex items-center gap-1.5 mt-2 w-max">
                              <FlaskConical size={12} className="text-indigo-600 shrink-0" />
                            ...
                            <div className="space-y-1.5">
                              ...
                              {ref?.stages && (
                                <div className="text-[11px] text-slate-500 font-medium">
                                  Route: Sub-Centre ➔ PHC ➔ IGGGH Pondicherry
                                </div>
                              )}
                            </div>
                          ) : (
                            ...
                            isClosed ? (
                              <div className="bg-emerald-50 border border-emerald-200 p-2.5 rounded-xl space-y-1">
                                ...
                                <p className="text-xs text-emerald-900 leading-snug font-medium">
                                  {record.closeNote || (ref && ref.stages[ref.stages.length - 1].note) || "Treatment completed at tertiary hospital."}
                                </p>
                                <span className="text-[10px] text-emerald-600 block">
                                  {record.closeDate ? new Date(record.closeDate).toLocaleDateString() : "Verified Closed"}
                                </span>
                              </div>
                            ) : (
                              <div className="bg-amber-50 border border-amber-200 p-2.5 rounded-xl space-y-2">
                                ...
                                <p className="text-[11px] text-amber-900 leading-snug">
                                  {ref?.stages.find((s: any) => s.status === "current")?.note || "Under review at Indira Gandhi Govt General Hospital, Pondicherry"}
                                </p>
                                <button
                                  type="button"
                                  onClick={() => {
                                    ...
                                    setClosureNote("Patient examined and stabilized. Referral concluded with PHC follow-up.")
                                  }}
                                  className="w-full text-xs font-bold text-white bg-amber-600 hover:bg-amber-700 py-1.5 px-2.5 rounded-lg shadow-xs transition-colors cursor-pointer"
                                >
                                  Close Referral Now
                                </button>
                              </div>
      ...
      {closingRefId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white rounded-3xl shadow-2xl max-w-lg w-full overflow-hidden animate-in zoom-in-95 duration-200 border border-slate-200">
            <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-teal-50/80">
              <div>
                <h3 className="text-lg font-extrabold text-slate-900">Close Outbound Referral</h3>
            ...
            <form onSubmit={handleCloseReferralSubmit} className="p-6 space-y-4">
              <div>
                <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5">
                  Discharge & Consultation Closure Note *
                </label>
                <textarea
                  required
                  rows={3}
                  ...
                  placeholder="e.g. Patient successfully treated at IGGGH, reports no residual acute symptoms. Discharged back to Villianur PHC."
                  className="w-full p-3 border border-slate-200 rounded-xl text-xs font-medium focus:outline-none focus:ring-2 focus:ring-teal-500"
                />
              </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  <button
                    ...
                    className="text-[11px] font-bold text-teal-700 bg-teal-50 hover:bg-teal-100 border border-teal-200 px-2 py-0.5 rounded cursor-pointer"
                  >
                    ⚡ Use PT-8891
                  </button>
                </div>
                <input
                  ...
                  className="w-full p-3 border border-slate-200 rounded-xl text-sm font-mono font-bold uppercase tracking-widest bg-slate-50 focus:outline-none focus:ring-2 focus:ring-teal-500"
                />
              </div>
